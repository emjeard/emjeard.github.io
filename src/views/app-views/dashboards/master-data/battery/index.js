...
const BatteryIndex = () => {
  ...
  const retrieveData = (page, many, filter) => {
    ...
    getListBattery(page, many, filter)
      .then((response) => {
  ...
  const showCreate = () => {
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          postBattery(store.getState().gen_hp_data.data.data_label).then(
            (response) => {
    ...
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          ...
          delBattery(item_id).then((response) => {
            if (response.status === 200) {
              toast.success(response.data.message, {
                position: "top-right",
                autoClose: true,
          ...
          dataItem.map((items, index) => (
            ...
            >
              <ItemModel
                ...
                onDeleteItem={(id, jbat) => deleteItem(id, jbat)}
                onUpdateItem={(id, jbat) => updateItem(id, jbat)}
              />
            </div>
          ))}
        {firstLoading === false ? (
