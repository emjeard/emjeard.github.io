...
const MemoryCardIndex = () => {
  ...
  const retrieveData = (page, many, filter) => {
    ...
    getListMemoryType(page, many, filter)
      .then((response) => {
  ...
  const showCreate = () => {
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          postMemoryType(store.getState().gen_hp_data.data.data_label).then(
            (response) => {
    ...
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          ...
          delMemoryType(item_id).then((response) => {
            if (response.status === 200) {
              toast.success(response.data.message, {
                position: "top-right",
                autoClose: true,
          ...
          dataItem.map((items, index) => (
            ...
            >
              <ItemModel
                ...
                onDeleteItem={(id, memjen) => deleteItem(id, memjen)}
                onUpdateItem={(id, memjen) => updateItem(id, memjen)}
              />
            </div>
          ))}
        {firstLoading === false ? (
...
export default MemoryCardIndex;
