...
const ProvinceIndex = () => {
  ...
  const retrieveData = (page, many, filter) => {
    ...
    getListProvinsi(page, many, filter)
      .then((response) => {
  ...
  const showCreate = () => {
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          postProvinsi(store.getState().gen_hp_data.data.data_label).then(
            (response) => {
    ...
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          ...
          delProvinsi(item_id).then((response) => {
            if (response.status === 200) {
              toast.success(response.data.message, {
                position: "top-right",
                autoClose: true,
          ...
          dataItem.map((items, index) => (
            ...
            >
              <ItemModel
                ...
                onDeleteItem={(id, province) => deleteItem(id, province)}
                onUpdateItem={(id, province) => updateItem(id, province)}
              />
            </div>
          ))}
        {firstLoading === false ? (
