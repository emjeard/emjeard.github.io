...
const ResolutionIndex = () => {
  ...
  const retrieveData = (page, many, filter) => {
    ...
    getListResolution(page, many, filter)
      .then((response) => {
  ...
  const showCreate = () => {
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          postResolution(store.getState().gen_hp_data.data.data_label).then(
            (response) => {
    ...
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          ...
          delResolution(item_id).then((response) => {
            if (response.status === 200) {
              toast.success(response.data.message, {
                position: "top-right",
                autoClose: true,
          ...
          dataItem.map((items, index) => (
            ...
            >
              <ItemModel
                ...
                onDeleteItem={(id, fitkam) => deleteItem(id, fitkam)}
                onUpdateItem={(id, fitkam) => updateItem(id, fitkam)}
              />
            </div>
          ))}
        {firstLoading === false ? (
...
export default ResolutionIndex;
