...
const MemoryCapacityIndex = () => {
  ...
  const retrieveData = (page, many, filter) => {
    ...
    getListMemoryCap(page, many, filter)
      .then((response) => {
  ...
  const showCreate = () => {
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          postMemoryCap(store.getState().gen_hp_data.data.data_label).then(
            (response) => {
    ...
    confirm({
      ...
      async onOk() {
        return new Promise((resolve, reject) => {
          ...
          delMemoryCap(item_id).then((response) => {
            if (response.status === 200) {
              toast.success(response.data.message, {
                position: "top-right",
                autoClose: true,
          ...
          dataItem.map((items, index) => (
            ...
            >
              <ItemModel
                ...
                onDeleteItem={(id, memkap) => deleteItem(id, memkap)}
                onUpdateItem={(id, memkap) => updateItem(id, memkap)}
              />
            </div>
          ))}
        {firstLoading === false ? (
...
export default MemoryCapacityIndex;
